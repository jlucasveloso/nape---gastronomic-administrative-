...
class _AnaliseScreenState extends State<AnaliseScreen> {
  ...
  Future<_Periodo?> _abrirSeletorPeriodo(_Periodo atual) async {
  ...
  Map<String, double> _calcularCustos(AppState appState, DateTimeRange intervalo) {
    ...
    for (final d in appState.despesas) {
    ...
    for (final p in appState.pagamentos) {
  ...
  Map<String, double> _calcularDesempenho(
      AppState appState, DateTimeRange intervalo) {
    final vendas = appState.vendas
        .where((v) =>
    ...
    if (vendas.isEmpty) {
      return {'ticketMedio': 0, 'margemPorVenda': 0, 'totalVendas': 0};
    }

    final totalVendas = vendas.fold(0.0, (s, v) => s + v.valorTotal);
    final qtdTransacoes = vendas.length.toDouble();
    final ticketMedio = totalVendas / qtdTransacoes;

    final custos = _calcularCustos(appState, intervalo);
    final totalCustos =
        custos.values.fold(0.0, (s, v) => s + v);
    final margemPorVenda = qtdTransacoes > 0
        ? (totalVendas - totalCustos) / qtdTransacoes
        : 0.0;

    return {
      'ticketMedio': ticketMedio,
      'margemPorVenda': margemPorVenda,
  ...
  List<MapEntry<String, double>> _topProdutos(
    ...
    for (final v in appState.vendas) {
  ...
  Map<DateTime, double> _agruparPorDia(
      AppState appState, DateTimeRange intervalo, String tipo) {
  ...
  Widget build(BuildContext context) {
    ...
    final desempenho = _calcularDesempenho(appState, intervalo);
    final topProdutos = _topProdutos(appState, intervalo);

    final dadosFat = _agruparPorDia(appState, intervalo, 'faturamento');
    final dadosCus = _agruparPorDia(appState, intervalo, 'custos');
    ...
    final vendasA = appState.vendas
        .where((v) => !v.data.isBefore(intervaloA.start) && !v.data.isAfter(intervaloA.end))
        .toList();
    final despesasA = appState.despesas
        .where((d) => !d.data.isBefore(intervaloA.start) && !d.data.isAfter(intervaloA.end))
        .toList();
    final controllerA = DashboardController(vendas: vendasA, despesas: despesasA);
    final custosA = _calcularCustos(appState, intervaloA);
    ...
    final margemA = controllerA.faturamentoTotal > 0
        ? (lucroA / controllerA.faturamentoTotal) * 100
        : 0.0;

    final vendasB = appState.vendas
        .where((v) => !v.data.isBefore(intervaloB.start) && !v.data.isAfter(intervaloB.end))
        .toList();
    final despesasB = appState.despesas
        .where((d) => !d.data.isBefore(intervaloB.start) && !d.data.isAfter(intervaloB.end))
        .toList();
    final controllerB = DashboardController(vendas: vendasB, despesas: despesasB);
    final custosB = _calcularCustos(appState, intervaloB);
    ...
    final margemB = controllerB.faturamentoTotal > 0
        ? (lucroB / controllerB.faturamentoTotal) * 100
        : 0.0;
            ...
            child: ListView(
              ...
              children: [
                ...
                GestureDetector(
                  onTap: () async {
                    final resultado =
                        await _abrirSeletorPeriodo(_periodoSelecionado);
                ...
                _Secao(
                  ...
                  child: Column(
                    children: [
                      ...
                      const _Divisor(),
                      _LinhaSimples(
                          label: 'Custos fixos', valor: fmt.format(fixo)),
                      const _Divisor(),
                      _LinhaSimples(
                          label: 'Outros', valor: fmt.format(outros)),

                      // Expandir — dados cirúrgicos
                      const SizedBox(height: 12),
                      GestureDetector(
                        onTap: () => setState(
                            () => _custosExpandido = !_custosExpandido),
                        child: Row(
                      ...
                      if (_custosExpandido) ...[
                        ...
                        _DadoCircurgico(
                          ...
                          valor: intervalo.duration.inDays > 0
                              ? fmt.format(
                                  fixo / intervalo.duration.inDays)
                              : '—',
                        ),
                      ],

                      if (_graficoCO && dias.isNotEmpty) ...[
                        const SizedBox(height: 16),
                        _GraficoCustos(
                          appState: appState,
                          intervalo: intervalo,
                    ...
                    children: [
                      Row(
                        children: [
                          _CardMetrica(
                            label: 'Ticket médio',
                            valor: fmt.format(desempenho['ticketMedio']!),
                            cor: const Color(0xFF2D74C4),
                          ),
                          const SizedBox(width: 12),
                          _CardMetrica(
                            ...
                            valor: fmt
                                .format(desempenho['margemPorVenda']!),
                            cor: desempenho['margemPorVenda']! >= 0
                                ? const Color(0xFF3E8E41)
                                : const Color(0xFFC2463C),
                          ),
                        ],
                      ...
                      if (topProdutos.isNotEmpty) ...[
                        ...
                        ...topProdutos.asMap().entries.map((e) {
                          ...
                          final perc = receitaBruta > 0
                              ? (produto.value / receitaBruta * 100)
                                  .toStringAsFixed(1)
                              : '0';
                          return Padding(
                    ...
                    children: [
                        ...
                        children: [
                          Expanded(
                            child: _SeletorComparacao(
                              ...
                              onTap: () async {
                                final r = await _abrirSeletorPeriodo(
                                    _comparacaoA);
                                if (r == null) return;
                      ...
                      _LinhaComparacao(
                        ...
                        variacaoPerc: _variacao(
                          controllerA.faturamentoTotal,
                          controllerB.faturamentoTotal,
                        ),
                      ),
                      const _Divisor(),
                      _LinhaComparacao(
        ...
        child: Row(
          ...
          children: [
            Icon(
              ...
              size: 14,
              color:
                  visivel ? Colors.white : const Color(0xFFC2463C),
            ),
            const SizedBox(width: 4),
            Text(
...
class _GraficoCustos extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final itens = [
      _ItemPizza('CMV', custos['cmv']!, const Color(0xFFC2463C)),
      _ItemPizza('CMO', custos['cmo']!, const Color(0xFF2D74C4)),
      _ItemPizza('Fixos', custos['fixo']!, const Color(0xFFFFB300)),
      _ItemPizza('Outros', custos['outros']!, Colors.grey),
                ...
                SizedBox(
                  ...
                  child: ClipRRect(
                    ...
                    child: LinearProgressIndicator(
                      ...
                      valueColor:
                          AlwaysStoppedAnimation<Color>(item.cor),
                      minHeight: 6,
                    ),
                  ),
                ),
...
class _ItemPizza {
  ...
  final double valor;
  final Color cor;
  const _ItemPizza(this.label, this.valor, this.cor);
}
